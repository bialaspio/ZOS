...
$zapytanie = $baza_danych->prepare("select distinct imie from osoba o WHERE imie like :part");
$partImie = $partImie.'%';
$zapytanie->bindParam(':part', $partImie);
$zapytanie->execute();
$przewidywane_imiona = $zapytanie->fetchAll(PDO::FETCH_ASSOC);
// Sprawdzamy, czy wartość znajduje się w tabeli

if ($przewidywane_imiona){
	$json = json_encode($przewidywane_imiona);
	echo $json;
}
else{
