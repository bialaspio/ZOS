...
if (!$nip) {
    ...
    echo json_encode(['error' => 'Brak wymaganego parametru: nip']);
    exit;
}

try {
    // Przygotowanie zapytania SQL
    $stmt = $dbh->prepare("SELECT 1 FROM firmy WHERE nip = :nip");
    $stmt->bindParam(':nip', $nip, PDO::PARAM_STR);

    // Wykonanie zapytania
    $stmt->execute();
    $nip_jest = $stmt->fetch(PDO::FETCH_ASSOC);

    // Zwrócenie wyniku
    echo json_encode(['exists' => (bool) $nip_jest]);
