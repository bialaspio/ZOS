...
$NazwaNipFirma = $_POST["NazwaNipFirma"] ?? null;

if (!$NazwaNipFirma) {
    http_response_code(400); // Zwróć kod błędu 400 w przypadku braku danych wejściowych
    echo json_encode(['error' => 'Brak wymaganego parametru: NazwaNipFirma']);
    exit;
}

try {
    // Przygotowanie zapytania SQL
    $sql = "SELECT f.id FROM firmy f WHERE nazwa || '  NIP:' || nip LIKE :nazwanipfirma";
    $zapytanie = $baza_danych->prepare($sql);

    // Bindowanie parametrów
    $NazwaNipFirma = '%' . $NazwaNipFirma . '%';
    $zapytanie->bindParam(':nazwanipfirma', $NazwaNipFirma, PDO::PARAM_STR);

    // Wykonanie zapytania
    $zapytanie->execute();
    $przewidywane_id_firmy = $zapytanie->fetchAll(PDO::FETCH_ASSOC);

    // Zwrócenie wyników w formacie JSON
    echo json_encode($przewidywane_id_firmy ?: []);
