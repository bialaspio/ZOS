...
if (!$Imie || !$Nazwisko) {
    ...
    echo json_encode(['error' => 'Brak wymaganych parametrów: Imie, Nazwisko']);
    exit;
}

try {
    // Przygotowanie zapytania SQL
    $sql = "SELECT DISTINCT o.nazwisko FROM osoba o WHERE o.imie LIKE :imie AND o.nazwisko LIKE :nazwisko";
    $zapytanie = $baza_danych->prepare($sql);

    // Bindowanie parametrów
    $tmp_nazwisko = $Nazwisko . '%';
    $tmp_imie = $Imie . '%';
    $zapytanie->bindParam(':nazwisko', $tmp_nazwisko, PDO::PARAM_STR);
    $zapytanie->bindParam(':imie', $tmp_imie, PDO::PARAM_STR);

    // Wykonanie zapytania
    $zapytanie->execute();
    $przewidywane_nazwiska = $zapytanie->fetchAll(PDO::FETCH_ASSOC);

    // Zwrócenie wyników w formacie JSON
    echo json_encode($przewidywane_nazwiska ?: []);
