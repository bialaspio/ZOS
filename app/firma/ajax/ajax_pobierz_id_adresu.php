...
$KodAdrNier = $_POST["KodAdr"] ?? null;
$MiejscowoscAdrNier = $_POST["MiejscowoscAdr"] ?? null;
$UlicaAdrNier = $_POST["UlicaAdr"] ?? null;
$NrAdrNier = $_POST["NrAdr"] ?? null;
...
if (!$KodAdrNier || !$MiejscowoscAdrNier || !$NrAdrNier) {
    ...
    echo json_encode(['error' => 'Brak wymaganych parametrów: KodAdr, MiejscowoscAdr, NrAdr']);
    exit;
}
...
try {
    ...
    // Wykonanie zapytania
    $zapytanie->execute();
    $przewidywane_id_adresu = $zapytanie->fetchAll(PDO::FETCH_ASSOC);

    // Zwrócenie wyników w formacie JSON
    echo json_encode($przewidywane_id_adresu ?: []);
