...
$partKodPocz = $_POST["NazwaNipFirma"] ?? null;
...
try {
    ...
    $sql = "
        SELECT DISTINCT Nazwa || '  NIP:' || nip AS nazwa_nip 
        FROM firmy 
        WHERE kod_pocztowy LIKE :part
    ";
    $zapytanie = $baza_danych->prepare($sql);

    // Bindowanie parametrów
    $partKodPocz = $partKodPocz . '%';
    $zapytanie->bindParam(':part', $partKodPocz, PDO::PARAM_STR);
