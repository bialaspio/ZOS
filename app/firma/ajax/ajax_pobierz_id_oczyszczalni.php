...
$NazwaOczyszczalni = $_POST["NazwaOczyszczalni"] ?? null;

if (!$NazwaOczyszczalni) {
    http_response_code(400); // Zwróć kod błędu 400 w przypadku braku danych wejściowych
    echo json_encode(['error' => 'Brak wymaganego parametru: NazwaOczyszczalni']);
    exit;
}

try {
    // Przygotowanie zapytania SQL
    $sql = "SELECT o.id_oczysz FROM public.oczyszczalnia o WHERE o.nazwa_oczysz LIKE :NazwaOczyszczalni";
    $zapytanie = $baza_danych->prepare($sql);

    // Bindowanie parametrów
    $NazwaOczyszczalni = '%' . $NazwaOczyszczalni . '%';
    $zapytanie->bindParam(':NazwaOczyszczalni', $NazwaOczyszczalni, PDO::PARAM_STR);
