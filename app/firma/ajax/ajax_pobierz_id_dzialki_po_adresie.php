...
$KodAdrNier = $_POST["KodAdrWlSzamba"] ?? null;
$MiejscowoscAdrNier = $_POST["MiejscowoscAdrWlSzamba"] ?? null;
$UlicaAdrNier = $_POST["UlicaAdrWlSzamba"] ?? null;
$NrAdrNier = $_POST["NrAdrWlSzamba"] ?? null;

if (!$KodAdrNier || !$MiejscowoscAdrNier || !$NrAdrNier) {
...
try {
    ...
    if (empty($UlicaAdrNier) || $UlicaAdrNier === $MiejscowoscAdrNier) {
        $sql = "
            SELECT d.id_dzialki 
            FROM dzialki d 
            INNER JOIN dzialki_adresy da ON d.ogc_fid = da.id_dzialka 
            INNER JOIN adresy a ON a.ogc_fid = da.id_adres  
        ...
        ";
        $zapytanie = $baza_danych->prepare($sql);
        $zapytanie->bindParam(':ulica', $UlicaAdrNier, PDO::PARAM_STR);
    }

    // Bindowanie parametrów
    $zapytanie->bindParam(':nr_bud', $NrAdrNier, PDO::PARAM_STR);
    $zapytanie->bindParam(':kodpocz', $KodAdrNier, PDO::PARAM_STR);
    $zapytanie->bindParam(':miejs', $MiejscowoscAdrNier, PDO::PARAM_STR);

    // Wykonanie zapytania
    $zapytanie->execute();
    $przewidywane_id_dzialki = $zapytanie->fetchAll(PDO::FETCH_ASSOC);

    // Zwrócenie wyników w formacie JSON
    echo json_encode($przewidywane_id_dzialki ?: []);
