...
$NipFirma = $_POST["NipFirma"] ?? null;

if (!$NipFirma) {
    http_response_code(400); // Zwróć kod błędu 400 w przypadku braku danych wejściowych
    echo json_encode(['error' => 'Brak wymaganego parametru: NipFirma']);
    exit;
}

try {
    // Przygotowanie zapytania SQL
    $sql = "SELECT f.id FROM firmy f WHERE nip LIKE :nipfirma";
    $zapytanie = $baza_danych->prepare($sql);

    // Bindowanie parametrów
    $zapytanie->bindParam(':nipfirma', $NipFirma, PDO::PARAM_STR);
